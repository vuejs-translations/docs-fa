...
در Composition API، زمانی که از [`useTemplateRef()`‎](/api/composition-api-helpers#usetemplateref) <sup class="vt-badge" data-text="3.5+" /> استفاده می‌شود، ریفرنس‌های قالب (template refs) از طریق ارسال مقدار رشته‌ای به عنوان پراپ به vnode ایجاد می‌گردند.
...
<details>
<summary>نحوه استفاده پیش از نسخه 3.5</summary>

در نسخه‌های پیش از 3.5 که تابع `useTemplateRef()`‎ معرفی نشده بود، ارجاع‌های قالب (template refs) با ارسال مستقیم `ref()`‎ به‌عنوان یک prop به vnode ایجاد می‌شدند:
