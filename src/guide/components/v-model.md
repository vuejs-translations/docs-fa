...
وقتی درحال یادگیری اتصال‌های input در form‌ها بودیم، مشاهده کردیم که `v-model` دارای [پیراینده‌های داخلی](/guide/essentials/forms#modifiers) `‎.trim` و `‎.number` و `‎.lazy` بود. گاهی اوقات همچنین ممکن است بخواهید که `v-model`‌ای که روی کامپوننت سفارشی‌شده input خود قرار دادید هم قابلیت پشتیبانی از پیراینده‌های سفارشی را داشته باشد.
