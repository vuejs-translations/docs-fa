...
از نسخه 3.4 به بعد در vue، یک Computed تنها زمانی اِفِکت خود را فراخوانی می‌کند که مقدار محاسبه‌شده آن نسبت به قبل تغییر کرده باشد. به عنوان مثال، `isEven` زیر تنها در صورتی افکت را فراخوانی می‌کند که مقدار برگردانده‌شده از `true` به `false` تغییر کند یا برعکس:
