...
- `<Transition>` برای اعمال انیمیشن‌ها بر روی یک عنصر یا کامپوننت هنگام ورود به DOM و یا خروج از آن است. این موضوع در همین صفحه بررسی شده است.
...
[انیمیشن‌های بومی CSS](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Animations/Using_CSS_animations) به مشابه با ترنزیشن‌های CSS استفاده می‌شوند، با این تفاوت که `‎*-enter-from` فوراً پس از وارد شدن عنصر حذف نمی‌شود، بلکه در رویداد `animationend` حذف می‌شود.
