...
## تغییردهنده‌های دکمه ماوس {#mouse-button-modifiers}
...
با این حال، توجه داشته باشید که نام‌های تغییر دهنده‌ی `‎.left` ، `‎.right` و `‎.middle` بر اساس چیدمان معمول موس برای افراد راست‌دست تعیین شده‌اند. اما در واقع، این نام‌ها به ترتیب نشان‌دهنده‌ی رویدادهای دستگاه‌های اشاره‌گر یعنی "اصلی"، "فرعی" و "کمکی" هستند و به دکمه‌های فیزیکی مربوط نمی‌شوند. بنابراین، در چیدمان موس چپ‌دست، ممکن است دکمه "اصلی" فیزیکی همان دکمه راست باشد، ولی تغییر دهنده‌ی `‎.left` را فعال کند. برای مثال، یک ترک‌پد ممکن است با یک ضربه یک‌انگشتی تغییر دهنده‌ی `‎.left`، با یک ضربه دو انگشتی تغییر دهنده‌ی `‎.right` و با یک ضربه سه‌انگشتی تغییر دهنده‌ی `‎.middle` را فعال کند. به طور مشابه، دستگاه‌ها و منابع رویدادی دیگری که رویدادهای "موس" تولید می‌کنند، ممکن است روش‌های فعال‌سازی داشته باشند که اصلاً به "چپ" و "راست" ربطی نداشته باشند.
